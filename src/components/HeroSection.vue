...
<template>
  <section id="hero" class="relative overflow-hidden bg-ink text-paper">
    <!-- ambient shapes -->
    <div
      class="pointer-events-none absolute -top-24 -right-24 w-96 h-96 rounded-full bg-gold/20 blur-3xl animate-float-a"
    />
    <div
      class="pointer-events-none absolute bottom-0 -left-24 w-80 h-80 rounded-full bg-aqua/20 blur-3xl animate-float-b"
    />
    <div
      class="pointer-events-none absolute top-1/3 left-1/3 w-64 h-64 rounded-full bg-gradient-primary opacity-10 blur-3xl animate-float-b-slow"
    />
    ...
    >
      <div>
        ...
        <div
          class="text-paper/70 text-lg leading-relaxed mb-10 max-w-lg"
          v-html="renderMarkdown(hero.paragraph)"
          v-reveal="240"
        ></div>
        ...
        <div class="flex flex-col sm:flex-row items-center gap-4" v-reveal="320">
          <a
            ...
            class="inline-flex items-center justify-center gap-2 bg-gradient-primary text-ink font-semibold px-6 py-3.5 rounded-full hover:brightness-105 hover:shadow-lg hover:shadow-gold/20 hover:scale-105 active:scale-95 transition whitespace-nowrap"
          >
      ...
      <div class="relative group" v-reveal="{ delay: 200, type: 'scale' }">
        <NatureImage
          :url="images.hero.url"
          :alt="hero.photoCaption || images.hero.alt"
          variant="dark"
          imgClass="img-zoom"
        />
        <blockquote
          v-if="hero.quote"
          class="mt-6 border-l-2 border-gold pl-5 font-display font-bold italic text-lg md:text-xl text-paper/90"
        >
          „{{ hero.quote }}"
          <cite
            v-if="hero.quoteAuthor"
            class="block mt-3 text-base text-paper/60 not-italic font-body"
            >— {{ hero.quoteAuthor }}</cite
          >
        </blockquote>
      </div>
    </div>
