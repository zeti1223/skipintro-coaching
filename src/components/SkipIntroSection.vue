<script setup>
import { skipIntro } from '../content.js'
    ...
    <div class="max-w-6xl mx-auto px-6">
      <div class="grid md:grid-cols-2 gap-8 items-start mb-6">
        <div v-reveal="{ type: 'left' }">
          ...
          <div
            class="text-body/85 leading-relaxed mb-8 max-w-2xl"
            v-html="renderMarkdown(skipIntro.intro)"
          ></div>

          <div
            class="text-body/85 leading-relaxed mb-8 max-w-2xl"
            v-html="renderMarkdown(skipIntro.conclusion)"
          ></div>
        </div>

        <div v-if="skipIntro.image" v-reveal="{ type: 'right', delay: 100 }">
          <div class="rounded-card overflow-hidden border border-aqua/40 group">
            <img
          ...
          <blockquote
            v-if="skipIntro.quote"
            class="mt-10 border-l-2 border-gold pl-5 font-bold font-display italic text-lg md:text-2xl text-ink/90"
          >
            „{{ skipIntro.quote }}"
          </blockquote>
        </div>
